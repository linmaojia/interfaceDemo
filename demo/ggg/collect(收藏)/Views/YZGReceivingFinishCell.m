...
@interface YZGReceivingFinishCell ()
...
@implementation YZGReceivingFinishCell

+ (YZGReceivingFinishCell *)workNewsCellWithCollectionView:(UICollectionView *)collectionView forIndexPath:(NSIndexPath *)indexPath{
    YZGReceivingFinishCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([self class]) forIndexPath:indexPath];
    if (cell == nil) {
        cell = [[YZGReceivingFinishCell alloc] init];
    }
    return cell;
...
- (void)setModel:(ProductDetail *)model{
    ...
    NSURL *imgUrl = [NSURL URLWithString:[[model.imgPath stringByAppendingString:@"@!product-list"] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
